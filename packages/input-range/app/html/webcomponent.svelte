<svelte:options tag="hb-input-range" />
...
<script lang="ts">
	...
	$: {
		if (typeof schemaentry === "string") {
			console.info("rendering schemaentry as string for hb-input-range");
			try {
				schemaentry = JSON.parse(schemaentry);
			} catch (err) {
				console.error("error parsing JSON for schemaentry hb-input-range", err);
			}
		}
		if (
